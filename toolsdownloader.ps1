...
$Groups = [ordered]@{
    ...
    'Eric Zimmerman' = @(
        ...
        'https://download.ericzimmermanstools.com/net9/AppCompatCacheParser.zip'
    ...
    'Generic Tools' = @(
        ...
        'https://github.com/Speedyxxxx/AltChecker/raw/refs/heads/main/AltChecker.exe'
...
Write-Host "  ${Grey}Output folder  ${White}$ssFolder${Reset}"
Write-Host "  ${Grey}Total tools    ${White}$totalTools${Reset} ${Gray}across $($Groups.Count) groups${Reset}"
Write-Host ""
...
Write-Host "    ${White}[A]${Grey}  All tools ${Gray}($totalTools files)${Reset}"
Write-Host "    ${White}[C]${Grey}  Choose specific groups${Reset}"
...
elseif ($mode -eq 'C') {
    ...
    for ($i = 0; $i -lt $groupKeys.Count; $i++) {
        ...
        Write-Host "    ${White}[$($i + 1)]${Grey} $($groupKeys[$i]) ${Gray}($cnt tools)${Reset}"
    }

    Write-Host ""

    Write-Host "  ${White}Enter group numbers separated by commas ${Gray}(e.g. 1,3,5)${White}:${Reset}"
    ...
    foreach ($part in ($raw -split ',')) {
        ...
        if ($part -match '^\d+$') {
            ...
            if ($idx -ge 0 -and $idx -lt $groupKeys.Count) {
                ...
                if ($selectedNames -notcontains $groupKeys[$idx]) {
...
Write-Host "  ${White}Creating ${Grey}$ssFolder...${Reset}" -NoNewline
...
Write-Host "  ${White}Tools saved to ${White}$ssFolder${Reset}"
